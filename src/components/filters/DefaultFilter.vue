<template>
  <q-card class="q-pa-md">
    ...
    <q-card-section>
      <q-chip
        v-for="(chip, index) in chipsWithLabels"
        :key="index"
        removable
      ...
      <div class="row q-pt-md">
        <q-select v-model="selectedFields.field"
                  :options="fieldOptions"
                  label="Campo"
                  map-options
                  class="col-3" />
        <q-space></q-space>
        <q-select v-if="selectedFields.field != ''"
                  v-model="selectedFields.operator"
        ...
        >
          <template v-slot:option="props">
            <q-item v-if="props['opt'].category.includes(selectedFields.field['type'])"
        ...
        <q-space />
        <component :is="getFieldComponent()"
                   v-model="selectedFields.value"
                   class="col-3"
                   :model-date="selectedFields.value"
                   @update:model-date="onDateChange"
        />
      </div>
      <div class="row q-pt-md">
        <div class="col-auto">
          <q-btn v-if="selectedFields.value != ''"
                 @click="addField"
                 color="primary"
        ...
        <div class="col"></div>
        <div class="col-auto">
          <q-btn v-if="chipsWithLabels?.length > 0"
                 label="Limpar"
                 text-color="white"
                 @click="clearSearch"
...
const getFieldComponent = () => {
  if (selectedFields.value.field['type'] === 'date') {
    if (selectedFields.value.operator === '<>') {
      return DateInputRange;
    }
    return DateInput;
  } else if (selectedFields.value.field['type'] === 'boolean') {
    return QToggle;
  } else if (selectedFields.value.field['type'] === 'string') {
    return QInput;
  }
};
